...
> v0.3.0 蓝白 Aurora 运维台：侧边导航、KPI 统计卡、SSO / 账号结果折叠面板。
...
| 浏览器模式 | 有头 / 无头 | 调试用有头；Linux 无桌面用 Xvfb，勿硬开无头 |
| 浏览器代理 | `http://…` 或 `socks5://…` | 有代理建议填；Docker 勿写宿主机 `127.0.0.1` |
...
2. **Turnstile 外置优先**：`YESCAPTCHA_KEY` 或本地 `http://127.0.0.1:5072` solver。`turnstile_provider=external` / `strict_external` 时 **不允许** 回退 Chrome（服务器零浏览器）；`auto` 才允许浏览器回退
3. **可选 post-init**：注册成功后用 SSO 调 TOS / 生日 API（`protocol_post_init`）。SSO 会同时写入 `.x.ai` 与 `.grok.com`。这与设置里的「注册后打开 grok.com 做 Web 激活」不是同一条路径——后者只在 **browser** 后端里打开网页做人机/CF；协议路径默认不打开 grok.com，需要 CF 出口时仍用「批量补激活」
4. 环境被 Cloudflare 拦截时 **abort 且不消耗 alias 重试**
5. 成功日志拆分：`transport=http|browser turnstile=yescaptcha|local_solver|browser sso_follow=http|browser|cookie`（不要只看旧的 `mode=pure-http`）

启用协议 Worker 前请确认：

1. 出口可访问 `accounts.x.ai`（可用 `browser_proxy` / `GROK_PROXY` / 系统 `HTTPS_PROXY`）
2. **服务器零浏览器**：`GROK_REGISTER_TURNSTILE_PROVIDER=external` + YesCaptcha 或本机 solver；YesCaptcha 会把注册代理写入 `TurnstileTask` 以绑定同出口
3. 桌面混合模式可用 `auto` 并保留有头 Chrome 作为回退
4. 并发保持 `1`
...
GROK_REGISTER_ALLOW_BROWSER_FALLBACK=false  # 可选；external 默认即为 false
```

相关模块：`core/registration/backend.py`、`turnstile.py`、`protocol_worker.py`。不要将本地 Solver 暴露到公网。
