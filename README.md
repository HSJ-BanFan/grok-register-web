...
- **多邮箱服务** — Microsoft Graph / Outlook REST、DuckMail、YYDS、Cloudflare Temp Email、Cloud Mail API
...
|----------|----------------|----------|
| `microsoft` | 使用账号库内 OAuth RefreshToken，通过 Graph 或旧 Outlook REST 收信；支持加号别名 | 先在邮箱管理页授权或导入账号 |
...
邮箱 provider 只负责创建邮箱和读取验证码。打开注册页、填写邮箱/OTP/资料、Turnstile、SSO、Web 激活及 grok2api 上传均复用同一套注册流程，不按邮箱服务商分叉。
...
| 注册后打开 grok.com 做 Web 激活 | 仅浏览器路径：注册成功后是否打开 grok.com 做人机/CF；与协议 pure-HTTP post-init 无关 | **关闭** |
...
- 选择 Microsoft 时，Token 会自动识别 Graph 与旧 Outlook REST 授权范围；无读取权限时会直接显示 OAuth/API 错误。程序不会在一次已发码的注册中途切换 provider
